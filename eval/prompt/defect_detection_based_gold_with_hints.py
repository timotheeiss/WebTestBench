...
This application has been annotated with **semantic hints** (`data-semtag-*` attributes), and you have a dedicated **Semantic Hints MCP** that reads them. Use it as your primary, low-cost way to perceive the UI. Use the **Playwright MCP** to perform actions. The two work together: *observe with Semantic Hints, act with Playwright.*
...
- **Every folded element is still individually clickable and observable.** Substitute into `idPattern`: item `beta` is `[data-semtag-id='products.grid.item.beta']`, its price is `products.grid.item.beta.price`, its buy button `products.grid.item.beta.buy`. Use those ids directly with `browser_click` / `semantic_observe`.
...
Pick the element's `id` from `semantic_snapshot`, then target it by the stable selector `[data-semtag-id='<id>']`. The Playwright action tools (`browser_click`, `browser_type`, `browser_select_option`, `browser_fill_form`) accept this CSS selector directly as their `target` — **no** `browser_snapshot` and no element ref needed, e.g.:

- Click an action: `browser_click` with `target: "[data-semtag-id='checkout.submit']"`.
- Fill one input: `browser_type` with `target: "[data-semtag-id='filters.search']"`, `text: "laptop"`.
- Fill a whole form in ONE call: `browser_fill_form` with a `fields` list where each field's `target` is a `[data-semtag-id='<id>']` selector. Never fill a form one `browser_type` call per field.
- Choose a dropdown/select option (these are custom selects, not native `<select>`): open the trigger, then click the option by its id — never `browser_snapshot` the menu:
  1. `browser_click` on `[data-semtag-id='filters.category']`
  2. `browser_click` on `[data-semtag-id='filters.category.option.tech']`
- Repeat an interaction many times (e.g. clicking the same button N times) or read bulk DOM state: use ONE `browser_evaluate` loop instead of many separate clicks, e.g. `() => { const b = document.querySelector("[data-semtag-id='tickets.item.t1.increment']"); for (let i = 0; i < 20; i++) { if (b.disabled) return { stoppedAt: i, disabled: true }; b.click(); } return { done: true }; }`.
...
1. Click the trigger by its `data-semtag-id` to open the menu.
2. Click `[data-semtag-id='<trigger-id>.option.<value>']`.
...
Only take a `browser_snapshot` when the element you need has **no** `data-semtag-id` (then act on it by ref), or when the hint channel is genuinely insufficient.
...
- Act with Playwright by passing the `[data-semtag-id='<id>']` selector directly to the action tools (no snapshot needed).
...
- Fallback: If a checklist item concerns an element with no `data-semtag-id`, or `semantic_snapshot`/`semantic_observe` is insufficient, fall back to `browser_snapshot` and the ref-based Playwright tools for that item only.
...
4. Execute: Perform the action with Playwright, targeting `[data-semtag-id='<id>']`.
